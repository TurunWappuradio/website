...
const MusicLibrary = () => {
    ...
    const filteredList = tracklist.filter((track) => {
        return ( track.artist.toLowerCase().match(state.toLowerCase()) || track.title.toLowerCase().match(state.toLowerCase()) ); 
    });
    ...
    <div>
        ...
        <div class="TracklistSearchContainer">
            <input class="TracklistSearch" onChange={changeValue} placeholder="Suodata kappaleita..."/>
        </div>
        <ul>
            {filteredList.map( (track, index) => (
            <li key={index} class="TracklistItem">{track.artist} - {track.title}</li>
            ))}
        </ul>
    </div>
